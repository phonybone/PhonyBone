...
@EXPORT_OK = qw(unique_sorted unique all_unique union intersection 
		...
		is_monotonic_asc is_monotonic_desc least greatest range end reverse
...
sub least {
    ...
    my $min=$list->[0];
    local $SIG{__WARN__}=sub {confess @_} if $ENV{DEBUG};
    foreach my $e (@$list) {
	$min=$e if $e < $min;
    }
    $min;
...
# return the min and max values contained in a list[ref]
sub range {
    my $list=(ref $_[0] eq 'ARRAY' && scalar @_ == 1)? shift : \@_;
    confess "not a listref: $list" unless ref $list eq 'ARRAY';
